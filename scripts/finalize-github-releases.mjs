...
if (result.schemaVersion !== 1 || !Array.isArray(result.releases)) {
  throw new Error("release result is malformed");
}
...
if (!result.ok) {
  for (const message of result.unresolved ?? []) console.error(`- ${message}`);
  throw new Error("npm publication was incomplete; successful packages were tagged before this failure");
}
