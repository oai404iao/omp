# Historical bundle source notices

This is the source-attribution record before the bundle became composition-only.
Paths and bootstrap restrictions below describe that historical snapshot, not
the current package layout or release eligibility. Current ownership and notices
are in [source-map.md](source-map.md) and each owner package's
`THIRD_PARTY_NOTICES.md`. Immutable provenance identifiers are unchanged.
