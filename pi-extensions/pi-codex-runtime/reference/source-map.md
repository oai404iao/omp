...
The upstream revisions and evidence above are unchanged by the package split.
The bundle has no private source facades or mirrored assets. Follow these owners
when auditing or modifying code. Paths below are relative to each owner's
`src/`, except the composition entry at the bundle root.
...
| Default composition | pi-codex-minimal-tools | root `index.ts`, `subagent-inline.ts` |

Behavioral tests live in each owner's `tests/`; cross-capability activation,
presentation and composition tests live in repository-root `tests/codex/`.
Transport harnesses live in core's `tests/support/`; neutral shared fixtures are
in root `tests/codex/support/`. Owner tests must not load sibling capabilities.

Runtime exclusively publishes `config.schema.json`, `models.schema.json` and
the default catalog; core exclusively publishes the apply-patch grammar.
Each owner retains the license/NOTICE/provenance needed by its shipped code.
Immutable provenance `localPaths` describe the original snapshot and may name
retired bundle paths; they are evidence, not live imports.
The [historical source notices](legacy-source-notices.md) preserve the original
attribution and bootstrap record. Current notices live in each owner package.
The original split's [path migration table](historical-source-owners.json) is
retained as historical evidence, not a list of currently existing bundle files.
