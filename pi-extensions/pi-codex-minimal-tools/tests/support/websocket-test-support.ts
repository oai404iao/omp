...
export function successEvents(responseId: string, text?: string): unknown[] {
	const output = text
		? [{
				type: "message",
				id: `msg_${responseId}`,
...
export function encodeServerFrame(payload: string | Buffer, options?: { binary?: boolean }): Buffer {
...
export function encodeServerText(payload: unknown): Buffer {
	return encodeServerFrame(JSON.stringify(payload));
}

export function decodeClientFrames(buffer: Uint8Array): { frames: string[]; remaining: Buffer } {
...
export type WebSocketResponseSequence = WebSocketResponse | {
...
export async function readJsonRequest(request: IncomingMessage, response: ServerResponse): Promise<Record<string, any>> {
	const chunks: Buffer[] = [];
