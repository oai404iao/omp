...
import { requiredPolicyIds } from "./discovery.ts";

export interface CodeModeConfig { hostPath: string; protocol: ProtocolMode; visibility: VisibilityMode; maxCells: number; requiredPolicies: string[] }
export interface Configuration { value: CodeModeConfig; path: string; sources: Record<keyof CodeModeConfig, "default" | "config" | "CLI"> }
const keys = ["version", "hostPath", "protocol", "visibility", "maxCells", "requiredPolicies"];
...
export function configuration(pi: Pick<ExtensionAPI, "getFlag">, agentDir = getAgentDir()): Configuration {
	...
	try {
		...
		if (data.version !== 1 || Object.keys(data).some((key) => !keys.includes(key))) throw new Error("Expected version 1 and only hostPath/protocol/visibility/maxCells/requiredPolicies; grants cannot be saved here");
		...
		if ("requiredPolicies" in data) requiredPolicyIds(data.requiredPolicies);
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw new Error(`Invalid Code Mode configuration at ${path}`, { cause: error });
	}
	const sources: Configuration["sources"] = { hostPath: "default", protocol: "default", visibility: "default", maxCells: "default", requiredPolicies: "default" };
	if ("requiredPolicies" in data) sources.requiredPolicies = "config";
	...
	return { path, sources, value: {
		...
		maxCells, requiredPolicies: requiredPolicyIds(data.requiredPolicies ?? []),
	} };
}
